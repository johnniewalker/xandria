...
require_once( 'Xandria/AppEnvironment/EnvironmentType.php' );
...
class Xandria_MyErrorTrigger
	...
	*/
	static public function triggerGracefulDbDegradationWarning() 
		...
		//else
		Xandria_MyErrorTrigger::triggerMyError( 'Gracefully handling db connection error', E_USER_WARNING );
		}
	...
	*/
	static public function triggerGracefulSystemMisconfigurationWarning( $message )
		{		
		//else
		Xandria_MyErrorTrigger::triggerMyError( 'Gracefully handled config error - '. $message, E_USER_WARNING );
		}
	...
	*/
	static public function triggerAssertionFailedError( $message )
		{		
		//else
		Xandria_MyErrorTrigger::triggerMyError( 
	...
	*/
	static public function triggerUserlandDataIsUndefinedWarning( $message )
		{		
		//else
		Xandria_MyErrorTrigger::triggerMyError( 'Some userland data is undefined - ' . $message, E_USER_WARNING );
		}
	...
	*/
	static public function triggerDbSQLExceptionWarning( 
			Xandria_Db_SQLException $sQLExceptionObj
			) 
		{
		...
		if ( ( Xandria_AppEnvironment_EnvironmentType::isEnvVarSetToATrueValue() )
			&& (!(  Xandria_AppEnvironment_EnvironmentType::doesEnvVarIndicateProductionEnvironment() )) )
			{
		...
		*/
		$atomicLogger = new Xandria_Logs_AtomicLogInbox();
